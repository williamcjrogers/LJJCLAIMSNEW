...
function calculateSummary() {
  ...
  claimsHierarchy.summary = {
    ...
    strongest_claim: heads.reduce((a, b) => (a.successProbability > b.successProbability ? a : b))
      .id,
...
export const ClaimsHierarchyUtils = {
  ...
  calculateSummary() {
    ...
    summary.total_sub_claims = heads.reduce(
      (total, head) => total + (head.subClaims?.length || 0),
      0
    );
    summary.total_claim_value = heads.reduce((total, head) => total + (head.totalClaim || 0), 0);
    summary.average_strength =
      heads.reduce((total, head) => total + (head.evidenceStrength || 0), 0) / heads.length;

    // Find strongest and weakest claims
    const sortedByStrength = [...heads].sort(
  ...
  getHeadTimeline(headId) {
    ...
    return (
      claimsHierarchy.masterTimeline.filter(event => event.affectedHeads?.includes(headId)) || []
    );
  },
  ...
  getCombinedTimeline(headId) {
    return this.getMasterTimelineForHead(headId).sort(
