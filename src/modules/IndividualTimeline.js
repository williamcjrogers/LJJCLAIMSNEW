...
export class IndividualTimeline {
    ...
    renderSummaryCards() {
        ...
        const totalSubClaimValue = this.headOfClaim.subClaims.reduce((sum, sub) => sum + sub.claimAmount, 0);
        const avgEvidence = Math.round(
            this.headOfClaim.subClaims.reduce((sum, sub) => sum + sub.evidenceStrength, 0) / subClaimsCount
        );
        ...
        return `
            <div class="sub-claims-section">
                ...
                <div class="sub-claims-grid">
                    ${this.headOfClaim.subClaims.map(subClaim => `
                            ...
                            <div class="sub-claim-body">
                                ...
                                ${subClaim.items && subClaim.items.length > 0 ? `
                                    <div class="sub-claim-items">
                                        ...
                                        <ul class="items-list">
                                            ${subClaim.items.slice(0, 3).map(item => `
                                                <li>
    ...
    calculateSuccessFactors() {
        ...
        if (category.includes('Life Safety')) {
            ...
            analysis.complianceReasoning = 'Fire safety and life safety systems have strict regulatory requirements. Non-compliance identified.';
            ...
            analysis.legalReasoning = 'BMS commissioning defects are well-documented with clear contractual requirements.';
        } else if (category.includes('Electrical')) {
            analysis.complianceIssues = true;
            analysis.complianceReasoning = 'Multiple NICEIC compliance issues identified requiring remediation.';
        }

        // Evidence reasoning
        if (evidenceScore >= 85) {
            analysis.evidenceReasoning = 'Strong documentary evidence including test reports, certificates, and commissioning data.';
        } else if (evidenceScore >= 75) {
            analysis.evidenceReasoning = 'Good evidence base with installation records and defect reports.';
        } else {
            analysis.evidenceReasoning = 'Evidence available but additional documentation may strengthen the claim.';
        }
        ...
        // Overall reasoning
        analysis.overallReasoning = `This claim for ${category} has a ${this.headOfClaim.successProbability}% probability of success based on: ` +
