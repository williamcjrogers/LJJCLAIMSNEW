...
class ComprehensiveClaimSystem {
        ...
        this.caseData = {
            ...
            timeline: [
                ...
                },
                {
                    ...
                    description: 'LST missed BMS integration with CPGS systems, requiring reverse engineering attempts',
                    category: 'critical_failure',
                ...
                },
                {
                    ...
                    description: 'Paul Vino (Quinn Ross) identifies "a few dodgy issues" during BMS commissioning inspection',
                    category: 'critical',
                ...
                },
                {
                    ...
                    description: 'Low pressure problems in Service/DCWS systems documented, dating back to August 2021',
                    category: 'evidence',
                ...
                },
                {
                    ...
                    description: 'UL M&E Manager contacts K&T regarding BMS motors not being properly linked to control systems',
                    category: 'critical',
                ...
                },
                {
                    ...
                    description: 'K&T issues BMS completion report acknowledging outstanding integration issues',
                    category: 'evidence',
                    supporting_docs: ['kt_bms_completion_report_19_04_24.pdf', 'outstanding_issues_summary.pdf'],
    ...
    renderTimeline() {
        ...
        const sortedTimeline = [...this.caseData.timeline].sort((a, b) => new Date(a.date) - new Date(b.date));
                    ...
                    <div class="timeline-content">
                        ...
                        <div class="timeline-connections">
                            ...
                            <div class="connection-list">
                                ${this.getTimelineConnections(event, sortedTimeline).map(connection => `
        ...
        keywords.forEach(keyword => {
            if (currentEvent.event.includes(keyword) || currentEvent.description.includes(keyword)) {
                timeline.forEach((event, index) => {
                    if (event !== currentEvent && (event.event.includes(keyword) || event.description.includes(keyword))) {
                        connections.push({
    ...
    extractKeywords(text) {
        const commonWords = ['the', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'is', 'are', 'was', 'were', 'been', 'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would', 'could', 'should', 'may', 'might', 'must', 'shall', 'can', 'a', 'an'];
    ...
    performSemanticSearch(query) {
        ...
        if (query.toLowerCase().includes('total') && (query.toLowerCase().includes('claim') || query.toLowerCase().includes('value'))) {
        ...
        if (query.toLowerCase().includes('timeline') || query.toLowerCase().includes('chronological')) {
            results.relatedItems = this.searchIndex.timeline.filter(item => 
                queryKeywords.some(keyword => item.keywords.includes(keyword))
        ...
        entities.forEach(entity => {
            if (query.toUpperCase().includes(entity)) {
                const entityMatches = [
                    ...
                    item.title.toUpperCase().includes(entity) || 
                    item.content.toUpperCase().includes(entity)
                );
    ...
    calculateTotalClaims(query) {
        ...
        const weightedValue = breakdowns.reduce((sum, claim) => 
            sum + (claim.value * (claim.success_probability / 100)), 0
        );
    ...
    generateInsights(query, results) {
        ...
        const highImpactEvents = results.relatedItems
            .filter(item => item.type === 'timeline' && item.impact && item.impact.includes('high'))
            .length;
        ...
        const strongEvidence = results.relatedItems
            .filter(item => item.evidence_strength && item.evidence_strength >= 85)
            .length;
            ...
            html += `
                <div class="calculations-section">
                    ...
                    <div class="calculation-breakdown">
                        ...
                        ${results.calculations.breakdowns.slice(0, 5).map(claim => `
        ...
        if (results.relatedItems.length > 0) {
            html += `
                <div class="related-items-section">
                    ...
                    <div class="results-grid">
                        ${results.relatedItems.slice(0, 10).map(item => this.renderSearchResult(item)).join('')}
                    </div>
                </div>
            `;
        }
