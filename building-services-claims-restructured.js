...
const buildingServicesClaims = {
    ...
    'Mechanical Building Services': {
        ...
        description: 'Comprehensive mechanical system failures across 6 major subsystems affecting building performance, energy efficiency, and occupant comfort',
        ...
        sub_claims: {
            chlorination_systems: {
                ...
                description: 'Water chlorination system defects and Service/DCWS low pressure issues documented since August 2021, spanning over 2 years of performance problems',
                key_timeline_issues: [
                    {
                        date: '13/12/23',
    ...
    Electrical: {
        ...
        description: 'Comprehensive electrical system failures across 4 major subsystems including power distribution, lightning protection, security systems, and trace heating',
    ...
    'Life Safety Systems': {
        ...
        description: 'Critical life safety system failures across fire alarm, emergency generator, and smoke ventilation systems affecting building safety compliance',
...
buildingServicesClaims.TOTALS = {
    total_claim_value: Object.values(buildingServicesClaims).reduce((sum, folder) => 
        folder.folder_type === 'parent' ? sum + folder.total_claim_value : sum, 0),
    
    total_sub_claims: Object.values(buildingServicesClaims).reduce((sum, folder) => 
        folder.folder_type === 'parent' ? sum + Object.keys(folder.sub_claims).length : sum, 0),
        
    weighted_success_probability: Math.round(
        Object.values(buildingServicesClaims).reduce((sum, folder, index, arr) => {
            if (folder.folder_type === 'parent') {
                return sum + (folder.success_probability * folder.total_claim_value);
            }
            return sum;
        }, 0) / Object.values(buildingServicesClaims).reduce((sum, folder) => 
            folder.folder_type === 'parent' ? sum + folder.total_claim_value : sum, 0)
    )
};
